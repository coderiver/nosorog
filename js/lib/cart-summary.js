...
var updateValue = function(element) {
    ...
    var wrapper = element.closest('.cart_quantity');

    var decorator =$('input.pack', wrapper);
    if (decorator.length) {
        $(decorator[0]).val(value/pack + ' уп.');
    }

	wrapper.find('.val').html(value);
};
...
$(document).ready(function(){
    $(document).off('click', '.cart_quantity_up').on('click', '.cart_quantity_up', function(e) {
        ...
        var element = $(this);
        if (element.data('pack') && element.data('pack') > 1) {
            var input = $('input.cart_quantity_input', element.closest('.cart_quantity'));
            upQuantity(element.attr('id').replace('cart_quantity_up_', ''), element.data('pack'), updateValue);
        } else {
            upQuantity(element.attr('id').replace('cart_quantity_up_', ''));
        }
    });
    ...
    $(document).off('click', '.cart_quantity_down').on('click', '.cart_quantity_down', function(e) {
        ...
        if (element.data('pack') && element.data('pack') > 1) {
            var input = $('input.cart_quantity_input', element.closest('.cart_quantity'));
            downQuantity(element.attr('id').replace('cart_quantity_down_', ''), element.data('pack'), updateValue);
        } else {
            downQuantity(element.attr('id').replace('cart_quantity_down_', ''));
        }
    });
...
function upQuantity(id, qty, callback)
{
...
function downQuantity(id, qty, callback)
{
    var input = $('input[name=quantity_' + id + ']');
	var val = input.val();
	var newVal = val;
...
function updateCartSummary(json)
{
	...
	redrawCart({ nbTotalProducts: nbrProducts, isFixed: $('.shopping_cart.fixed').length });
