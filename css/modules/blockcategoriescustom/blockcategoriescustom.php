...
class BlockCategoriesCustom extends Module
{
    public function __construct()
    {
        $this->name = 'blockcategoriescustom';
        ...
        $this->displayName = $this->l('Categories block custom');
        $this->description = $this->l('Adds a block featuring product categories custom.');
    ...
    public function install()
    {
        ...
        $tab = new Tab();
        $tab->active = 1;
        $tab->class_name = "AdminBlockCategoriesCustom";
        $tab->name = array();
        ...
        if (!$tab->add() ||
            !parent::install() ||
            !$this->registerHook('topLeft') ||
            !$this->registerHook('displayTopLeftNotHome') ||
    ...
    {
        $id_tab = (int)Tab::getIdFromClassName('AdminBlockCategoriesCustom');
    ...
    public function hookDisplayBackOfficeCategory($params)
    {
        ...
        for ($i=0;$i<3;$i++)
        {
            if (file_exists(_PS_CAT_IMG_DIR_.(int)$category->id.'-'.$i.'_thumb.jpg'))
            {
                ...
                $files[$i]['delete_url'] = Context::getContext()->link->getAdminLink('AdminBlockCategoriesCustom').'&deleteThumb='.$i.'&id_category='.(int)$category->id;
            }
        }
        ...
        $helper->setMultiple(true)->setUseAjax(true)->setName('thumbnail')->setFiles($files)->setMaxFiles(3)->setUrl(
            Context::getContext()->link->getAdminLink('AdminBlockCategoriesCustom').'&ajax=1&id_category='.$category->id
            .'&action=uploadThumbnailImages');
        $this->smarty->assign('helper', $helper->render());
        return $this->display(__FILE__, 'views/blockcategoriescustom_admin.tpl');
    }
     ...
     * @return string
     */
    public function hookLeftColumn($params, $template='blockcategoriescustom_acordeon.tpl')
    {
        ...
        if (!$this->isCached('blockcategoriescustom.tpl', $cacheId))
        {
            $range = '';
            ...
            if (file_exists(_PS_THEME_DIR_.'modules/blockcategoriescustom/blockcategoriescustom.tpl'))
                $this->smarty->assign('branche_tpl_path', _PS_THEME_DIR_.'modules/blockcategoriescustom/category-tree-branch.tpl');
            else
                $this->smarty->assign('branche_tpl_path', _PS_MODULE_DIR_.'blockcategoriescustom/category-tree-branch.tpl');
        }
    ...
    {
        $cache_id = 'blockcategoriescustom::setLastVisitedCategory';
...
//    {
//        $this->setLastVisitedCategory();
//        if (!$this->isCached('blockcategoriescustom_footer.tpl', $this->getCacheId()))
//        {
//            $maxdepth = Configuration::get('BLOCK_CATEG_MAX_DEPTH');
...
//            $this->smarty->assign('blockCategTree', $blockCategTree);
//
//            if (file_exists(_PS_THEME_DIR_.'modules/blockcategoriescustom/blockcategoriescustom_footer.tpl'))
//                $this->smarty->assign('branche_tpl_path', _PS_THEME_DIR_.'modules/blockcategoriescustom/category-tree-branch.tpl');
//            else
//                $this->smarty->assign('branche_tpl_path', _PS_MODULE_DIR_.'blockcategoriescustom/category-tree-branch.tpl');
//            $this->smarty->assign('isDhtml', $isDhtml);
//        }
//        $display = $this->display(__FILE__, 'blockcategoriescustom_footer.tpl', $this->getCacheId());
//
//        return $display;
//    }
    ...
    public function hookDisplayTopLeftNotHome($params)
    {
        return $this->hookLeftColumn($params, 'blockcategoriescustom_dropdown.tpl');
    }

    public function hookHeader()
    {
        $this->context->controller->addJS(_THEME_JS_DIR_.'tools/treeManagement.js');
        $this->context->controller->addCSS(($this->_path).'blockcategoriescustom.css', 'all');
    }

    private function _clearBlockcategoriesCache()
    {
        $this->_clearCache('blockcategoriescustom.tpl');
//        $this->_clearCache('blockcategoriescustom_footer.tpl');
        $this->_clearCache('blockcategoriescustom_top.tpl');
        $this->_clearCache('blockcategoriescustom_dropdown.tpl');
        $this->_clearCache('category-tree-branch.tpl');
    }
